...
void screen_keyboard_validate_entered_text(void) {
    ...
    // copy output text
    memcpy(G_ux_params.u.keyboard.entered_text,
           G_bolos_ux_context.keyboard_user_buffer,
           sizeof(G_ux_params.u.keyboard.entered_text));
    // pop the screen
    ux_stack_pop();
    ux_stack_redisplay();
}
...
unsigned int screen_consent_ticker(unsigned int ignored) {
    ...
    G_bolos_ux_context.onboarding_index =
        (G_bolos_ux_context.onboarding_index + 1) % G_bolos_ux_context.onboarding_step;
...
void screen_consent_ticker_init(unsigned int number_of_steps,
    ...
    /*if (!check_pin_to_confirm || ! os_perso_isonboarded() == BOLOS_UX_OK) { managed at bolos task
     * level */
