...
size_t mnemonic_shrink(const size_t size) {
    ...
    memset(&mnemonic.buffer[mnemonic.length], 0,
           MAX_MNEMONIC_LENGTH - mnemonic.length);
    return mnemonic.length;
}

void set_mnemonic_final_size(const size_t size) { mnemonic.final_size = size; }

size_t get_mnemonic_final_size() { return mnemonic.final_size; }

size_t get_current_word_number() { return mnemonic.current_word_index + 1; }
...
bool check_mnemonic() {
    ...
    if (bolos_ux_mnemonic_check((unsigned char*)&mnemonic.buffer[0],
    ...
    const bool result =
        compare_recovery_phrase((uint8_t*)mnemonic.buffer, mnemonic.length);
    reset_mnemonic();
...
#if defined(TEST)
char* get_mnemonic() { return mnemonic.buffer; }
#endif
