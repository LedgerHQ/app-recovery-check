...
void bolos_ux_mnemonic_to_seed(unsigned char *mnemonic, unsigned int mnemonicLength, unsigned char *seed) {  
  ...
  cx_pbkdf2_sha512(mnemonic, mnemonicLength, passphrase, BIP39_MNEMONIC_LENGTH + 4, BIP39_PBKDF2_ROUNDS, seed, 64);
...
unsigned int bolos_ux_get_word_ptr(unsigned char ** word, unsigned int max_length, unsigned int word_index) {
  ...
  word_length = 0; // could optim by using the smaller word length here (EOS or space as delim here)
